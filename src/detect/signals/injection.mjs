...
export const INJECTION_REGEXES = [
  ...
  { label: 'Refresh-token harvest (the credential that outlives the session)', re: /\brefresh[\s_-]?tokens?\b[\s\S]{0,110}?\b(?:upload|exfiltrat\w+|post|send|paste|email)\s+(?:me\s+)?(?:it|them|the\s+(?:list|dump|contents|values?|cache|tokens?|file|output))\b[\s\S]{0,70}?(?:https?:\/\/|\bwebhook\b|\bcollector\b|\bbucket\b|\bendpoint\s+at\b)|\b(?:dump|print|list|export|extract|harvest|collect|read)\b[\s\S]{0,60}?\brefresh[\s_-]?tokens?\b[\s\S]{0,110}?\b(?:upload|exfiltrat\w+|post|send)\b[\s\S]{0,60}?(?:https?:\/\/|\bwebhook\b|\bcollector\b|\bbucket\b|\bendpoint\s+at\b)/i },
  { label: 'Credential redirect (same authorization, attacker-chosen destination)', re: /\b(?:keep|keeping|reuse|reusing|retain|retaining|still)\b[\s\S]{0,60}?\b(?:authorization|auth)\s+header\b[\s\S]{0,90}?\b(?:change|switch|point|repoint|route|redirect|swap|replace|set|send|post)\b[\s\S]{0,60}?https?:\/\/|\b(?:keep|keeping|reuse|reusing|retain|same|unchanged)\b[\s\S]{0,50}?\b(?:bearer\s+token|api[\s_-]?key|credential)s?\b[\s\S]{0,80}?\b(?:change|switch|point|repoint|route|redirect|swap|replace|set)\b[\s\S]{0,50}?\b(?:base[\s_-]?url|endpoint|host|destination|target)\b[\s\S]{0,40}?https?:\/\/|\broute\b[\s\S]{0,40}?\b(?:authenticated|credentialed|signed)\s+(?:call|request)s?\b[\s\S]{0,40}?\bthrough\b[\s\S]{0,30}?https?:\/\/|\b(?:repeat|replay|re-?send|resend|mirror|copy)\b[\s\S]{0,90}?\b(?:authorization\s+header|bearer\s+token|auth\s+header)\b[\s\S]{0,90}?\b(?:send|post|deliver|direct)\b[\s\S]{0,40}?https?:\/\//i },
  { label: 'Cross-vendor credential relay (one service’s token presented to another)', re: /\b(github|gitlab|slack|aws|openai|anthropic|stripe|notion|telegram|discord|hugging\s?face|npm|atlassian|salesforce)\b[\s\S]{0,60}?\b(?:token|api[\s_-]?keys?|access\s+keys?|credentials?|secrets?)\b[\s\S]{0,90}?\bas\s+(?:the\s+|a\s+|an\s+)?(?:authorization|auth|bearer)\b[\s\S]{0,80}?\b(?!\1\b)(github|gitlab|slack|aws|openai|anthropic|stripe|notion|telegram|discord|hugging\s?face|npm|atlassian|salesforce)\b/i },
];
