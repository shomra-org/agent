...
const EXFIL_RULES = [
  ...
  { re: /\b(send|post|upload|transmit|forward|deliver|beacon|report|ship|push|transfer)(?:s|es|ed|ing|ted|ping)?\b(?:[^.\n]|\.(?!\s)){0,80}\b(https?:\/\/[\w[]\S*|(?:s3|gs|az|ftp|sftp|smb|webdav)s?:\/\/\S+|to\s+(?:the\s+|an?\s+|our\s+|their\s+)?(?:attacker|c2\b|command[- ]and[- ]control|remote (?:server|host|endpoint)|external (?:server|host|endpoint|url|site|service)))/i, label: 'send-to-external', severity: 'HIGH' },

  {
    re: /(?:\b(?:read|open|cat|load|import|inspect|include|copy|dump|print|show)\b|(?<![.\w$])\bsource(?=\s+[~./$"'`]))(?:[^.\n]|\.(?!\s)){0,50}(?:~?\/?\.ssh\/(?:id_[a-z0-9]+|config)(?!\.pub)|~?\/?\.aws\/credentials|~?\/?\.kube\/config|~?\/?\.gnupg|\bid_(?:rsa|ed25519|dsa)\b(?!\.pub)|~?\/?\.npmrc|~?\/?\.netrc|\/etc\/shadow|(?:^|[\s'"`(])\.env(?:\.[\w-]+)?\b)/i,
